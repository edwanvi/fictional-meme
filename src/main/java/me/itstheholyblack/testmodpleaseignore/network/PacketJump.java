	...
	public static class Handler implements IMessageHandler<PacketJump, IMessage> {
		...
		public IMessage onMessage(PacketJump message, MessageContext ctx) {
			...
			player.mcServer.addScheduledTask(() -> {
				if(player.inventory.armorItemInSlot(0) != null && player.inventory.armorItemInSlot(0).getItem() instanceof LeapingBoots) {
					player.addExhaustion(0.3F);
					player.fallDistance = 0;
					
					// if(belt != null && belt.getItem() instanceof ItemTravelBelt)
					player.fallDistance = -((LeapingBoots) player.inventory.armorItemInSlot(0).getItem()).getMaxAllowedJumps();
				}
			});
			return null;
		}
